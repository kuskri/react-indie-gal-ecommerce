...
function SearchFeature(props) {
    ...
    return (
        <Box>
            ...
            <Input value={searchTerm} onChange={onChangeSearch} placeholder='Search product'></Input>
            {products.filter((product) => {
                if (searchTerm === '' || isReset){
                    return ''
                } else if (product.productName.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())){
                    return product;
                } else if (product.productBrand.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())){
                    return product;
                } else if (product.category.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())){
                ...
                <Box >
                    <Link to={`/products/${product._id}`} onClick={searchModalOnClose}>
                        <Flex p={2} key={key} onClick={onClose}>
                            ...
                            <Text  p={2} fontSize='sm' key={products._id} >{product.productName}</Text>
                        </Flex>
                    </Link>
                </Box>)
            })}
